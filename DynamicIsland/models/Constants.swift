...
extension Defaults.Keys {
    ...
    static let enableRealTimeWaveform = Key<Bool>("enableRealTimeWaveform", default: true)
    ...
    static let enableMinimalisticUI = Key<Bool>("enableMinimalisticUI", default: false)
        ...
        // MARK: Atoll Bridge (智能监控)
    /// Shared with the Python bridge daemon via ~/.config/atoll_bridge/config.json.
    /// The app rewrites the whole file whenever any bridge setting changes; the
    /// daemon re-reads it every poll cycle (3s cache).
    static let bridgeMonitorEnabled = Key<Bool>("bridgeMonitorEnabled", default: true)
    static let bridgeBreaksEnabled = Key<Bool>("bridgeBreaksEnabled", default: true)
    static let bridgeStandInterval = Key<Int>("bridgeStandInterval", default: 60)
    static let bridgeWaterInterval = Key<Int>("bridgeWaterInterval", default: 40)
    static let bridgeHardCpuThreshold = Key<Double>("bridgeHardCpuThreshold", default: 25)
    static let bridgeActiveGrace = Key<Double>("bridgeActiveGrace", default: 12)
    static let bridgeEvidenceWindow = Key<Double>("bridgeEvidenceWindow", default: 45)
    static let bridgeWeatherAlertsEnabled = Key<Bool>("bridgeWeatherAlertsEnabled", default: true)
    /// 强制常显天气：开启后收起态无视工作状态永远显示天气卡。
    /// 默认关闭 = 自动切换：工作 agent > 天气 > legacy 兜底。
    static let bridgeClosedShowWeather = Key<Bool>("bridgeClosedShowWeather", default: true)
    static let bridgePomodoroEnabled = Key<Bool>("bridgePomodoroEnabled", default: true)
