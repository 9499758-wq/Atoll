...
class DynamicIslandViewModel: NSObject, ObservableObject {
    ...
    func isMouseHovering(position: NSPoint = NSEvent.mouseLocation) -> Bool {
        ...
        if let frame = screenFrame {
            let liveWindowFrame = AppDelegate.shared?.window?.frame
            let hitWidth = max(notchSize.width, liveWindowFrame?.width ?? 0)
            let hitHeight = max(notchSize.height, liveWindowFrame?.height ?? 0)
            let baseY = frame.maxY - hitHeight
            let baseX = frame.midX - hitWidth / 2
            
            return position.y >= baseY && position.x >= baseX && position.x <= baseX + hitWidth
        }
