...
struct PlaybackState {
    ...
    var playerLyrics: String?
    /// 当通过系统 MediaRemote 也取不到歌词时标记。
    /// true 表示「系统层面确实无歌词」（播放器未提供）；
    /// 用于区分「播放器本就不提供歌词（如汽水音乐）」与「尚未查询到」。
    var lyricsUnavailable: Bool = false
    var isExplicit: Bool?
...
extension PlaybackState: Equatable {
    static func == (lhs: PlaybackState, rhs: PlaybackState) -> Bool {
        return lhs.bundleIdentifier == rhs.bundleIdentifier
            ...
            && lhs.playerLyrics == rhs.playerLyrics
