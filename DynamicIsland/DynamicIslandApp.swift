...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("Atoll", systemImage: "sparkles", isInserted: $showMenuBarIcon) {
            Button("设置") {
...
final class FirstMouseHostingView<Content: View>: PassthroughHostingView<Content> {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    let atollBridgeLocationWriter = AtollBridgeLocationWriter.shared  // CoreLocation -> /tmp/atoll_bridge_location.json for the bridge daemon's weather card
...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
    ...
    private func syncNotchSpaceMembership() {
        // Do not attach the main island window to the private CGSSpace. On this
        // macOS 15 setup it can leave Atoll capturable by window ID while not
        // composited onscreen. Standard collectionBehavior handles all spaces.
        ...
        -> NSWindow
    {
        ...
        let content = FirstMouseHostingView(
            rootView: ContentView()
                .environmentObject(viewModel)
                .environmentObject(webcamManager)
                //.moveToSky()
        )
        // 让主窗口只命中有内容的圆角卡片区，圆角外透明区穿透点击（修复灵动岛正下方整块吃点击）
        let corner: CGFloat = shouldUseDynamicIslandMode(for: screen.localizedName) ? 24 : 26
        content.contentCornerRadius = corner
        window.contentView = content
        
        window.orderFrontRegardless()
        // Keep the main island on the normal window server path. The private
        // CGSSpace path can create a capturable-but-not-onscreen transparent
        // window on macOS 15 when the space membership gets stale.
        window.orderFrontRegardless()
    ...
    private func calculateRequiredNotchSize() -> CGSize {
        func withReactionSurface(_ size: CGSize) -> CGSize {
            guard let surface = ReactionAnimationPlayer.shared.activeSurfaceSize else { return size }
        ...
        if isInlineSneakPeekActive {
            ...
            return withReactionSurface(CGSize(width: inlineSneakPeekWidth, height: vm.effectiveClosedNotchHeight))
        }
        ...
        if vm.notchState == .closed && 
           ...
           Defaults[.showPowerStatusNotifications] {
            ...
            if let kind = batteryModel.activeTemporaryHUDKind {
                ...
                return withReactionSurface(addShadowPadding(to: CGSize(width: width, height: height), isMinimalistic: Defaults[.enableMinimalisticUI]))
            }
        }
        ...
        return withReactionSurface(result)
    }
    ...
    private func resizeWindow(_ window: NSWindow, on screen: NSScreen, to size: CGSize, animated: Bool) {
        ...
        DispatchQueue.main.async { [weak self, weak window] in
            ...
            self.positionWindow(window, on: screen, changeAlpha: false)
        }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Setup status manager only when explicitly enabled in preferences.
        ...
        ReactionAnimationPlayer.shared.$active
    ...
    private func installTopMenuItemsIfNeeded() {
        ...
        if let focusMenuItem = mainMenu.items.first(where: { $0.identifier?.rawValue == "Atoll.Focus.Menu" }) {
            mainMenu.removeItem(focusMenuItem)
        }
        if mainMenu.items.contains(where: { $0.identifier?.rawValue == "Atoll.Accessibility.Menu" }) {
            return
        }
        ...
        mainMenu.insertItem(accessibilityMenuItem, at: insertionIndex)
        ...
        mainMenu.insertItem(permissionsMenuItem, at: insertionIndex + 1)
        ...
        mainMenu.insertItem(toolsMenuItem, at: insertionIndex + 2)
    }
