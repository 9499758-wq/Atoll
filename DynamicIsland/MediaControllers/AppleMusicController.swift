...
class AppleMusicController: MediaControllerProtocol {
    ...
    func updatePlaybackInfo() async {
        ...
        if let lyricsString = descriptor.atIndex(10)?.stringValue,
           !lyricsString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            updatedState.playerLyrics = lyricsString.trimmingCharacters(in: .whitespacesAndNewlines)
        } else {
            updatedState.playerLyrics = nil
        }
    ...
    private func fetchPlaybackInfoAsync() async throws -> NSAppleEventDescriptor? {
        ...
        tell application "Music"
            try
                ...
                return {playerState, currentTrackName, currentTrackArtist, currentTrackAlbum, trackPosition, trackDuration, shuffleState, repeatValue, artData, trackLyrics}
            on error
