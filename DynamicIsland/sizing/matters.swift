...
var openNotchSize: CGSize {
    // Respect the tab-driven minimum width and the current display width.
    // A previous universal 480pt cap made the expanded bridge/status view
    // look like a cropped hover preview: system readouts and the recovery
    // command were squeezed into a narrow column. The screen clamp below is
    // the real guard against over-wide notches.
    let storedWidth = Defaults[.openNotchWidth]
    let minWidth = currentRecommendedMinimumNotchWidth()
    let width = min(max(storedWidth, minWidth), maxAllowedNotchWidth())
