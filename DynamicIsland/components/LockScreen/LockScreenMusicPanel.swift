...
struct LockScreenMusicPanel: View {
    ...
    private var shouldShowInlineLyrics: Bool {
        enableLyrics && !showsDetachedFullscreenLyrics && !musicManager.currentLyrics.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }
        ...
        if !rawLine.isEmpty {
            ...
            HStack(spacing: 8) {
                Image.cuteSymbol("music.note")
                    ...
                    .foregroundColor(widgetAppearance.primary(opacity: 0.72))
            ...
            .background(Capsule(style: .continuous).fill(widgetAppearance.primary(opacity: 0.08)))
            .padding(.top, isExpanded ? 12 : 8)
            .frame(maxWidth: .infinity, alignment: alignment)
            .animation(.smooth(duration: 0.32), value: rawLine)
        }
    }
    ...
    private var volumeSlider: some View {
        HStack(spacing: 14) {
            Image.cuteSymbol(volumeIconName)
                        ...
                        ForEach(airPlayManager.devices) { device in
                            VStack(spacing: 4) {
                                Button {
                                    ...
                                    HStack(spacing: 8) {
                                        Image.cuteSymbol(device.iconName)
...
private struct PanelControlButton: View {
    ...
    private var iconView: some View {
        let base = Image.cuteSymbol(icon)
