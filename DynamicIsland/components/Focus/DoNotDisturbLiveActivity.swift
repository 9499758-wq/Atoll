...
struct DoNotDisturbLiveActivity: View {
    ...
    private var labelText: String {
        if focusToastMode {
            return manager.isDoNotDisturbActive ? "开启" : "关闭"
        }
    ...
    private var accessibilityDescription: String {
        if manager.isDoNotDisturbActive {
            return labelText.isEmpty ? "状态已开启" : "状态已开启：\(labelText)"
        } else {
            return "状态已关闭"
        }
    }
    ...
    private var inactiveIconMatchingActiveStyle: Image {
        ...
        if focusMode == .gaming,
           SymbolAvailabilityCache.shared.isSymbolAvailable("rocket.circle.fill") {
            return Image.cuteSymbol("rocket.circle.fill")
        }
        ...
        return Image.cuteSymbol(focusMode.inactiveSymbol)
    }
