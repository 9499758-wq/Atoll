...
private enum SettingsTab: String, CaseIterable, Identifiable {
    ...
    var group: SettingsTabGroup {
        ...
        case .atollBridge, .timer, .calendar, .notes:                        return .productivity
    ...
    var title: String {
        ...
        case .atollBridge: return String(localized: "智能监控")
    ...
    var systemImage: String {
        ...
        case .atollBridge: return "sparkles.rectangle.stack"
    ...
    var tint: Color {
        ...
        case .atollBridge: return Color(red: 0.16, green: 0.83, blue: 1.0)
...
struct SettingsView: View {
    ...
    private func sidebarIcon(for tab: SettingsTab) -> some View {
        RoundedRectangle(cornerRadius: 8, style: .continuous)
            ...
            .overlay {
                Image.cuteSymbol(tab.systemImage)
    ...
    private struct SettingsSidebarSearchBar: View {
        ...
        private var searchField: some View {
            HStack(spacing: 6) {
                Image.cuteSymbol("magnifyingglass")
                ...
                if !text.isEmpty {
                    Button {
                        ...
                        Image.cuteSymbol("xmark.circle.fill")
                            .foregroundStyle(Color.secondary)
                    }
                    .buttonStyle(.plain)
                    ...
                    Button {
                        ...
                        HStack(spacing: 10) {
                            RoundedRectangle(cornerRadius: 8, style: .continuous)
                                ...
                                .overlay {
                                    Image.cuteSymbol(suggestion.tab.systemImage)
                                        .font(.system(size: 13, weight: .semibold))
                                        .foregroundStyle(Color.white)
                                }
    ...
    private var settingsSearchIndex: [SettingsSearchEntry] {
        [
            ...
            // Atoll Bridge 智能监控
            SettingsSearchEntry(tab: .atollBridge, title: "启用智能监控", keywords: ["智能监控", "agent", "monitor", "bridge", "hermes", "codex", "claude"], highlightID: SettingsTab.atollBridge.highlightID(for: "启用智能监控")),
            SettingsSearchEntry(tab: .atollBridge, title: "久坐/喝水提醒", keywords: ["久坐", "喝水", "提醒", "break", "stand", "water", "健康"], highlightID: SettingsTab.atollBridge.highlightID(for: "久坐/喝水提醒")),
            SettingsSearchEntry(tab: .atollBridge, title: "久坐提醒间隔", keywords: ["久坐", "间隔", "分钟", "stand interval"], highlightID: SettingsTab.atollBridge.highlightID(for: "久坐提醒间隔")),
            SettingsSearchEntry(tab: .atollBridge, title: "喝水提醒间隔", keywords: ["喝水", "间隔", "分钟", "water interval"], highlightID: SettingsTab.atollBridge.highlightID(for: "喝水提醒间隔")),
            SettingsSearchEntry(tab: .atollBridge, title: "天气预警", keywords: ["天气", "预警", "alert", "weather"], highlightID: SettingsTab.atollBridge.highlightID(for: "天气预警")),
            SettingsSearchEntry(tab: .atollBridge, title: "强制常显天气", keywords: ["天气", "收起", "常显", "自动切换", "weather", "closed", "强制"], highlightID: SettingsTab.atollBridge.highlightID(for: "强制常显天气")),
            SettingsSearchEntry(tab: .atollBridge, title: "工作判定阈值", keywords: ["cpu", "阈值", "threshold", "判定", "灵敏度"], highlightID: SettingsTab.atollBridge.highlightID(for: "工作判定阈值")),
        ]
    }
        ...
        Form {
            if BatteryActivityManager.shared.hasBattery() {
                ...
                ContentUnavailableView {
                    VStack(spacing: 16) {
                        Image.cuteSymbol("battery.100percent.slash")
                            .font(.title)
        ...
        VStack(spacing: 20) {
                ...
                HUDSelectionCard(
                    ...
                    VStack {
                        Capsule()
                            ...
                            .overlay {
                                HStack(spacing: 6) {
                                    Image.cuteSymbol(previewModel.iconName)
            ...
            case .osd:
                if #available(macOS 15.0, *) {
                    ...
                    VStack(spacing: 16) {
                        Image.cuteSymbol("exclamationmark.triangle.fill")
                            .font(.system(size: 48))
            ...
            if !calendarManager.hasCalendarAccess || !calendarManager.hasReminderAccess {
                ...
                Section {
                    Defaults.Toggle(key: .enableThirdPartyCalendarApp) {
                        HStack {
                            Image.cuteSymbol("ellipsis.calendar")
                                .foregroundStyle(.secondary)
            ...
            Form {
                ...
                HStack(spacing: 30) {
                    ...
                    Button {
                        ...
                        VStack(spacing: 5) {
                            Image.cuteSymbol("cup.and.saucer.fill")
                ...
                Section {
                    ForEach(UpdateChannel.availableChannels) { channel in
                        Button {
                            ...
                            HStack(spacing: 10) {
                                Image.cuteSymbol(channel.badgeIcon)
                                    .font(.system(size: 13))
    ...
    struct BluetoothHUDIconStyleCard: View {
        ...
        private var preview: some View {
            Group {
                switch style {
                case .symbol:
                    Image.cuteSymbol(BluetoothAudioDeviceType.airpods.sfSymbol)
                ...
                if !clipboardManager.clipboardHistory.isEmpty {
                    Section {
                        ForEach(clipboardManager.clipboardHistory) { item in
                            VStack(alignment: .leading, spacing: 4) {
                                HStack {
                                    Image.cuteSymbol(item.type.icon)
                ...
                if !screenAssistantManager.attachedFiles.isEmpty {
                    Section {
                        ForEach(screenAssistantManager.attachedFiles) { file in
                            VStack(alignment: .leading, spacing: 4) {
                                HStack {
                                    Image.cuteSymbol(file.type.iconName)
                                        .foregroundColor(.blue)
                                        .frame(width: 16)
    ...
    var body: some View {
        Group {
            if let nsImage = resolvedIcon() {
                ...
                Image.cuteSymbol(symbolFallback)
...
// MARK: - Atoll Bridge 智能监控 Settings

/// Shared-config writer. Every change to a bridge setting rewrites the whole
/// ~/.config/atoll_bridge/config.json consumed by the Python bridge daemon.
enum AtollBridgeConfigWriter {
    static func writeToFile(
        monitorEnabled: Bool, breaksEnabled: Bool,
        standInterval: Int, waterInterval: Int,
        hardCpuThreshold: Double, activeGrace: Double,
        evidenceWindow: Double, weatherAlertsEnabled: Bool,
        pomodoroEnabled: Bool = true
    ) {
        let dict: [String: Any] = [
            "monitorEnabled": monitorEnabled,
            "breaksEnabled": breaksEnabled,
            "standIntervalMinutes": standInterval,
            "waterIntervalMinutes": waterInterval,
            "hardCpuThreshold": hardCpuThreshold,
            "activeGraceSeconds": activeGrace,
            "evidenceWindow": evidenceWindow,
            "weatherAlertsEnabled": weatherAlertsEnabled,
            "pomodoroEnabled": pomodoroEnabled,
        ]
        let dirURL = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/atoll_bridge", isDirectory: true)
        do {
            try FileManager.default.createDirectory(at: dirURL, withIntermediateDirectories: true)
            let data = try JSONSerialization.data(withJSONObject: dict, options: [.prettyPrinted, .sortedKeys])
            try data.write(to: dirURL.appendingPathComponent("config.json"), options: .atomic)
        } catch {
            NSLog("AtollBridgeConfig write failed: \(error.localizedDescription)")
        }
    }
}

private struct AtollBridgeSettings: View {
    @Default(.bridgeMonitorEnabled) private var monitorEnabled
    @Default(.bridgeBreaksEnabled) private var breaksEnabled
    @Default(.bridgeStandInterval) private var standInterval
    @Default(.bridgeWaterInterval) private var waterInterval
    @Default(.bridgeHardCpuThreshold) private var hardCpuThreshold
    @Default(.bridgeActiveGrace) private var activeGrace
    @Default(.bridgeEvidenceWindow) private var evidenceWindow
    @Default(.bridgeWeatherAlertsEnabled) private var weatherAlertsEnabled
    @Default(.bridgeClosedShowWeather) private var closedShowWeather
    @Default(.bridgePomodoroEnabled) private var pomodoroEnabled

    private func syncConfigFile() {
        AtollBridgeConfigWriter.writeToFile(
            monitorEnabled: monitorEnabled,
            breaksEnabled: breaksEnabled,
    ...
    var body: some View {
        Form {
            AtollBridgeLiveMonitorSection()

            AtollBridgeMonitorSection(
                monitorEnabled: $monitorEnabled,
                hardCpuThreshold: $hardCpuThreshold,
                evidenceWindow: $evidenceWindow,
            ...
            AtollBridgeWeatherSection(
                weatherAlertsEnabled: $weatherAlertsEnabled,
                closedShowWeather: $closedShowWeather,
                syncConfigFile: syncConfigFile
            )

            AtollBridgePomoSection(
                pomodoroEnabled: $pomodoroEnabled,
                syncConfigFile: syncConfigFile
            )
            ...
            Section {
                LabeledContent {
                    Text("~/.config/atoll_bridge/config.json")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                ...
                Text("修改即时生效：守护进程每 3 秒重读一次配置，无需重启。")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        }
...
/// 番茄钟计时设置。番茄钟关闭时顺手停掉进行中的表，
/// 否则刘海里会挂一个看不见的倒计时。
private struct AtollBridgePomoSection: View {
    @Binding var pomodoroEnabled: Bool
    var syncConfigFile: () -> Void

    var body: some View {
        Section {
            Toggle(isOn: $pomodoroEnabled) {
                LabeledContent("番茄钟计时") {
                    Text("刘海倒计时 · 计时期间暂停久坐提醒")
                }
            }
            .onChange(of: pomodoroEnabled) { _, on in
                if !on { try? "stop".write(toFile: "/tmp/atoll_bridge_pomo.cmd", atomically: true, encoding: .utf8) }
                syncConfigFile()
            }

            LabeledContent("充电管家") {
                Text("常插电保性能 · 满电仅显示已连接充电器")
            }
        } header: {
            Text("番茄钟 & 充电管家")
        }
...
// MARK: - Atoll Bridge 实时监控看板（App 自带设置页的 agent/天气/系统监控）

/// 与刘海同源的 /tmp/atoll_bridge_status.json 实时看板，每 3 秒刷新。
/// 把之前只在刘海展开态可见的 agent 监控、天气实况、系统状态搬进 App 自己的设置页。
private struct AtollBridgeLiveMonitorSection: View {
    @State private var status = AtollBridgeStatus.load()
    @State private var forecastDays: [AtollForecastDay] = []
    @State private var forecastLabel = ""
    @State private var topProcs: [AtollBridgeUIState.TopProc] = []
    private let timer = Timer.publish(every: 3, on: .main, in: .common).autoconnect()

    private struct ForecastFile: Decodable { let label: String; let days: [AtollForecastDay] }
    private struct TopFile: Decodable { let procs: [AtollBridgeUIState.TopProc] }

    private var agents: [AtollBridgeEntity] {
        (status.agents ?? []).sorted {
    ...
    private func uptimeText(_ seconds: Int) -> String {
        let s = seconds, h = s / 3600, m = (s % 3600) / 60
    ...
    var body: some View {
        Section {
            ...
            // agent 状态卡
            if !agents.isEmpty {
                ForEach(agents, id: \.title) { agent in agentRow(agent) }
            }
            // 系统状态
            if let sys = status.system {
                Divider()
                systemGrid(sys)
            }
            // 通知（cron/休息/预警）
            if let notices = status.notices, !notices.isEmpty {
                Divider()
                ForEach(notices, id: \.title) { n in noticeRow(n) }
            }
            // 进程 TOP
            if !topProcs.isEmpty {
                Divider()
            ...
            if bridgeIsStale {
                Label("守护进程已停更（>90s），请检查 atoll_bridge.py 是否在运行", systemImage: "exclamationmark.triangle.fill")
                    .font(.footnote)
                    .foregroundStyle(.orange)
            }
        } header: {
            HStack(spacing: 6) {
                Circle()
                    .fill(bridgeIsStale ? Color.orange : Color.green)
                    .frame(width: 8, height: 8)
                Text("实时状态")
                Spacer()
                Text(bridgeIsStale ? "已停更" : "每 3 秒刷新")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var weatherNowRow: some View {
        HStack(spacing: 10) {
            Image.cuteSymbol(status.weather?.symbol ?? "cloud.sun")
                .font(.system(size: 24, weight: .semibold))
                .foregroundStyle(Color(atollHex: status.weather?.accent ?? "#5BB6FF"))
                .frame(width: 36)
            VStack(alignment: .leading, spacing: 2) {
                Text(status.weather?.title ?? "天气")
                    .font(.body.weight(.semibold))
                Text(status.weather?.detail ?? "")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Spacer()
            Text("实况")
                ...
                .padding(.horizontal, 8)
                .padding(.vertical, 3)
                .background(Capsule().fill(Color(atollHex: status.weather?.accent ?? "#5BB6FF").opacity(0.14)))
        }
    }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 12) {
                ForEach(forecastDays.indices, id: \.self) { i in
                    let d = forecastDays[i]
                    VStack(spacing: 3) {
                        Text(dayLabel(d.date, today: i == 0))
                            .font(.caption2.weight(.semibold))
                            .foregroundStyle(i == 0 ? Color(atollHex: "#FFD66E") : .secondary)
                        Image.cuteSymbol(d.symbol)
                            .font(.system(size: 14, weight: .semibold))
                            .foregroundStyle(Color(atollHex: i == 0 ? "#FFD66E" : "#5BB6FF"))
                        Text("\(d.tmin)° \(d.tmax)°")
    ...
    private func dayLabel(_ date: String, today: Bool) -> String {
        if today { return "今天" }
        let parts = date.split(separator: "-").map(String.init)
        guard parts.count == 3, let m = Int(parts[1]), let day = Int(parts[2]) else { return date }
        return "\(m)/\(day)"
    }

    private func agentRow(_ a: AtollBridgeEntity) -> some View {
        let isActive = a.active ?? false
        let accent = Color(atollHex: a.accent ?? "#29D3FF")
        let secondary = Color(atollHex: a.secondary ?? "#28F0A8")
        return HStack(spacing: 10) {
            AtollCuteIcon(symbolName: a.symbol.isEmpty ? "robot" : a.symbol,
                          size: 28,
                          accent: accent,
                          secondary: secondary,
                          showsPlate: false,
            ...
            AtollBridgeActivityBars(accent: accent, secondary: secondary, active: isActive, rate: a.rate)
                .frame(width: 52)
        }
    }

    private func systemGrid(_ sys: AtollBridgeSystem) -> some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
            statCell("CPU", String(format: "%.0f%%", sys.cpuPercent ?? 0), "cpu")
            statCell("内存", String(format: "%.0f%%", sys.memoryUsedPercent ?? 0), "memorychip")
            statCell("电池", batteryText(sys), "battery.100percent")
            statCell("磁盘", String(format: "%.0fG 可用", sys.diskFreeGB ?? 0), "externaldrive")
            statCell("负载", String(format: "%.2f", sys.loadAvg ?? 0), "gauge.with.dots.needle.50percent")
        }
    }

    private func statCell(_ label: String, _ value: String, _ icon: String) -> some View {
        VStack(spacing: 3) {
    ...
    private func batteryText(_ sys: AtollBridgeSystem) -> String {
        let pct = sys.batteryPercent ?? 0
        if sys.charging == true { return "充电中 \(pct)%" }
        if let min = sys.batteryTimeRemaining, min > 0 {
            return "\(pct)% \(Int(min / 60))h\(Int(min) % 60)m"
        }
        return "\(pct)%"
    }

    private func noticeRow(_ n: AtollBridgeEntity) -> some View {
        Label(n.title, systemImage: n.kind == "alert" ? "exclamationmark.octagon.fill" : "bell.fill")
            .font(.footnote)
            .foregroundStyle(n.kind == "alert" ? Color.orange : Color.primary)
    ...
    private var topRows: some View {
        VStack(alignment: .leading, spacing: 5) {
            Text("CPU 占用 TOP")
                .font(.caption)
                .foregroundStyle(.secondary)
            ForEach(Array(topProcs.prefix(5).enumerated()), id: \.element.pid) { idx, p in
                HStack(spacing: 8) {
                    Text("\(idx + 1)")
                    ...
                    Text(String(format: "%.1f%%", p.cpu))
                        .font(.caption2.monospacedDigit())
                        .foregroundStyle(p.cpu > 50 ? Color.orange : .secondary)
                }
            }
        }
        .padding(.vertical, 2)
...
private struct AtollBridgeMonitorSection: View {
    @Binding var monitorEnabled: Bool
    @Binding var hardCpuThreshold: Double
    @Binding var evidenceWindow: Double
    let syncConfigFile: () -> Void
    ...
    var body: some View {
        Section("智能监控") {
            ...
            .onChange(of: monitorEnabled) { _, _ in syncConfigFile() }
            .settingsHighlight(id: SettingsTab.atollBridge.highlightID(for: "启用智能监控"))

            if !monitorEnabled {
                Text("关闭后刘海只显示天气/系统状态，AI 代理卡片隐藏，数据仍在后台采集。")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                LabeledContent {
                    Text("\(Int(hardCpuThreshold)) %/s")
                        .monospacedDigit()
                } label: {
                    Text("CPU 工作判定阈值")
                }
                Slider(value: $hardCpuThreshold, in: 5...60, step: 1) {
                    Text("CPU threshold")
                }
                .onChange(of: hardCpuThreshold) { _, _ in syncConfigFile() }
                Text("瞬时 CPU 高于此值判定为「工作中」。调低更灵敏，调高可避免后台索引误判。")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
            .settingsHighlight(id: SettingsTab.atollBridge.highlightID(for: "工作判定阈值"))

            VStack(alignment: .leading, spacing: 8) {
                LabeledContent {
                    Text("\(Int(evidenceWindow)) 秒")
                        .monospacedDigit()
                } label: {
                    Text("会话证据窗口")
                }
                Slider(value: $evidenceWindow, in: 15...120, step: 5) {
                    Text("Evidence window")
                }
                .onChange(of: evidenceWindow) { _, _ in syncConfigFile() }
                Text("会话存储在此时间内有写入即视为正在工作。")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        }
...
private struct AtollBridgeBreaksSection: View {
    @Binding var breaksEnabled: Bool
    @Binding var standInterval: Int
    @Binding var waterInterval: Int
    let syncConfigFile: () -> Void
    ...
    var body: some View {
        Section("健康提醒") {
            ...
            .onChange(of: breaksEnabled) { _, _ in syncConfigFile() }
            .settingsHighlight(id: SettingsTab.atollBridge.highlightID(for: "久坐/喝水提醒"))

            if breaksEnabled {
                Picker(selection: $standInterval) {
                ...
                .onChange(of: standInterval) { _, _ in syncConfigFile() }

                Picker(selection: $waterInterval) {
                    ForEach([20, 30, 40, 60], id: \.self) { m in
                        Text("\(m) 分钟").tag(m)
                    }
                } label: {
                    Text("喝水提醒间隔")
                }
                .onChange(of: waterInterval) { _, _ in syncConfigFile() }

                Text("离开电脑超过 5 分钟自动重新计时；番茄钟计时期间提醒暂停。")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        }
...
private struct AtollBridgeWeatherSection: View {
    @Binding var weatherAlertsEnabled: Bool
    @Binding var closedShowWeather: Bool
    let syncConfigFile: () -> Void
    ...
    var body: some View {
        Section("天气") {
            ...
            .onChange(of: weatherAlertsEnabled) { _, _ in syncConfigFile() }
            .settingsHighlight(id: SettingsTab.atollBridge.highlightID(for: "天气预警"))
            Text("暴雨/大风等预警通过中国天气网获取，目前覆盖昆明及周边区县。")
                .font(.footnote)
                .foregroundStyle(.secondary)

            Toggle(isOn: $closedShowWeather) {
                Text("强制常显天气")
            }
            .settingsHighlight(id: SettingsTab.atollBridge.highlightID(for: "强制常显天气"))
            Text("默认自动切换：AI 工作中显示工作动态，空闲时显示天气。开启后仅在没有提醒和工作动态时常显天气。")
                .font(.footnote)
                .foregroundStyle(.secondary)
        }
    }
