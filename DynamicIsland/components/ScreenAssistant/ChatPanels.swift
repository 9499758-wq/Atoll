        ...
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    HStack(spacing: 6) {
                        Image.cuteSymbol("arrow.counterclockwise")
                            .font(.caption)
                        Text("Reset Context")
                            .font(.caption)
                    }
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 16) {
                        if screenAssistantManager.chatMessages.isEmpty {
                            VStack(spacing: 20) {
                                Image.cuteSymbol("brain.head.profile")
                                    .font(.system(size: 60))
                            ...
                            if screenAssistantManager.isLoading {
                                HStack(spacing: 12) {
                                    CuteReactionView(kind: .agent, size: 30)
                                    Text("AI is thinking...")
                                        .font(.body)
                                        .foregroundColor(.secondary)
                                    Spacer()
                ...
                .onChange(of: screenAssistantManager.isLoading) { _, _ in
                    if screenAssistantManager.isLoading {
                        ReactionAnimationPlayer.shared.present(.agent)
...
struct ChatInputView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                HStack(spacing: 6) {
                    Image.cuteSymbol(iconForProvider(currentProvider))
                        .font(.caption)
            ...
            HStack(spacing: 12) {
                ...
                Button(action: openModelSelection) {
                    Image.cuteSymbol("brain.head.profile.fill")
                        .foregroundColor(.purple)
                        .font(.system(size: 20))
                }
                .buttonStyle(PlainButtonStyle())
                ...
                Button(action: sendMessage) {
                    Image.cuteSymbol("paperplane.fill")
...
struct StreamingChatMessageBubble: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            // User avatar
            if message.isFromUser {
                Image.cuteSymbol("person.circle.fill")
                    .font(.title2)
                    .foregroundColor(.blue)
                    .frame(width: 32, height: 32)
            } else {
...
struct ScreenshotOptionButton: View {
    ...
    var body: some View {
        Button(action: { onTap(type) }) {
            HStack(spacing: 12) {
                Image.cuteSymbol(type.iconName)
                    .foregroundColor(.blue)
                    .font(.system(size: 16))
