...
struct BatteryView: View {
    ...
    var body: some View {
        let totalWidth: CGFloat = max(24, batteryWidth)
        let bodyWidth: CGFloat = totalWidth - 3.5
        let bodyHeight: CGFloat = 12.5
        let fillFraction = max(0.0, min(1.0, CGFloat(levelBattery) / 100.0))
        let innerPadding: CGFloat = 1.8
        let innerWidth = max(2.0, (bodyWidth - innerPadding * 2) * fillFraction)
        let innerHeight = max(2.0, bodyHeight - innerPadding * 2)

        HStack(spacing: 1.5) {
            ZStack(alignment: .leading) {
                // 1. 纯矢量 Apple 电池外壳 (Apple Battery Shell)
                RoundedRectangle(cornerRadius: 3.5, style: .continuous)
                    .stroke(Color.white.opacity(0.65), lineWidth: 1.2)
                    .frame(width: bodyWidth, height: bodyHeight)

                // 2. 电池内部电量填充 (Battery Level Fill)
                RoundedRectangle(cornerRadius: 2.0, style: .continuous)
                    .fill(batteryColor)
                    .frame(width: innerWidth, height: innerHeight)
                    .padding(.leading, innerPadding)

                // 3. 充电/插电状态闪电角标 (Charging Bolt)
                if isCharging || isPluggedIn {
                    Image(systemName: isCharging ? "bolt.fill" : "powerplug.fill")
                        .font(.system(size: 8, weight: .bold))
                        .foregroundStyle(isCharging ? (levelBattery > 40 ? Color.black : Color.white) : Color.white)
                        .frame(width: bodyWidth, height: bodyHeight, alignment: .center)
                }
            }

            // 4. 电池正极小凸起 (Battery Terminal Tip)
