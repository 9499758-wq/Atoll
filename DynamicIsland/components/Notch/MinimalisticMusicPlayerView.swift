...
struct MinimalisticMusicPlayerView: View {
    ...
    var body: some View {
        if !musicManager.hasActiveSession {
            ...
            VStack(spacing: 0) {
                ...
                if enableLyrics && !rawLyrics.isEmpty {
                    lyricsView
                        .padding(.top, 10)
                }
    ...
    private var dynamicHeightSignature: Int {
        ...
        if enableLyrics && !musicManager.currentLyrics.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { signature += 1 }
    ...
    private func calculateDynamicHeight() -> CGFloat {
        ...
        let hasLyrics = enableLyrics && !musicManager.currentLyrics.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    ...
    private var lyricsView: some View {
        ...
        guard !line.isEmpty else { return AnyView(EmptyView()) }
        ...
        .background(
            Capsule(style: .continuous)
                ...
                .overlay(Capsule(style: .continuous).stroke(Color.white.opacity(0.12), lineWidth: 0.7))
        )
            ...
            HStack(spacing: separatorSpacing) {
                if let url = linkURL {
                    Button {
                        ...
                        AtollCuteIcon(
                            symbolName: didCopyLink ? "checkmark.circle.fill" : "link",
                            size: 15,
                            accent: didCopyLink ? Color.green : Color.white.opacity(0.85),
                            showsPlate: false
                        )
...
private struct MinimalisticReminderDetailsView: View {
    ...
    private var progressBar: some View {
        timelineProgressBar
            // m3: 播放面右上角 AirPods 电量角标（连接时才出现，不挤控制键）
            .overlay(alignment: .topTrailing) {
                if let dev = BluetoothAudioManager.shared.connectedDevices.last ?? BluetoothAudioManager.shared.lastConnectedDevice,
                   !musicManager.isLiveStream {
                    HStack(spacing: 3) {
                        AtollCuteIcon(
                            symbolName: dev.deviceType.sfSymbol,
                            size: 12,
                            accent: .secondary,
                            showsPlate: false
                        )
                        if let lvl = dev.batteryLevel {
                            Text("\(lvl)%")
                    ...
                    .onHover { _ in BluetoothAudioManager.shared.refreshConnectedDeviceBatteries() }
                }
            }
    }

    private var timelineProgressBar: some View {
        TimelineView(
            .animation(
    ...
    private var controlHUDRow: some View {
        HStack(alignment: .center, spacing: 10) {
            if !controlLeftIconName.isEmpty {
                AtollCuteIcon(
                    symbolName: controlLeftIconName,
                    size: 22,
                    accent: .white,
                    showsPlate: false
                )
            ...
            if !controlRightIconName.isEmpty {
                AtollCuteIcon(
                    symbolName: controlRightIconName,
                    size: 22,
                    accent: .white,
                    showsPlate: false
                )
...
private struct MinimalisticSquircircleButton: View {
    ...
    private func iconView() -> some View {
        let image = AtollCuteIcon(
            symbolName: icon,
            size: max(14, fontSize + 4),
