        ...
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                if enableAppleNotesSync {
                    AtollCuteIcon(
                        symbolName: appleNotesSync.isSyncing ? "arrow.triangle.2.circlepath" : "apple.logo",
                        size: 14,
                        accent: appleNotesSync.isSyncing ? .yellow : .white.opacity(0.45),
                        showsPlate: false
                    )
                ...
                if enableAppleNotesSync {
                    Button(action: onSync) {
                        AtollCuteIcon(
                            ...
                            size: 16,
                            accent: appleNotesSync.isSyncing ? .yellow : .white.opacity(0.6),
                            showsPlate: false
                        )
                ...
                if enableNoteSearch || enableNoteColorFiltering {
                    Button(action: { 
                        ...
                        AtollCuteIcon(
                            ...
                            size: 16,
                            accent: isSearchExpanded ? .blue : .white.opacity(0.6),
                            showsPlate: false
                        )
            ...
            if sortedNotes.isEmpty {
                VStack {
                    ...
                    HStack(spacing: 8) {
                        AtollCuteIcon(
                            symbolName: searchText.isEmpty ? "note.text" : "magnifyingglass",
                            size: 18,
                            accent: .secondary.opacity(0.4),
                            showsPlate: false
                        )
                    ...
                    HStack(spacing: isCompact ? 3 : 4) {
                        if enableNotePinning {
                            Button(action: onTogglePin) {
                                AtollCuteIcon(
                                    symbolName: note.isPinned ? "pin.slash.fill" : "pin.fill",
