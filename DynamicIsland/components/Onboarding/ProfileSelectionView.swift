...
struct ProfileSelectionView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            // Header
            VStack(spacing: 8) {
                Image.cuteSymbol("person.3.fill")
                    .font(.system(size: 50))
        ...
        Button(action: onTap) {
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    ...
                    if isSelected {
                        Image.cuteSymbol("checkmark.circle.fill")
                            .font(.system(size: 22))
                            .foregroundStyle(.linearGradient(colors: profile.gradient, startPoint: .topLeading, endPoint: .bottomTrailing))
...
func applyProfileSettings(_ profiles: Set<String>) {
    ...
    // Lyrics enabled by default so the media panel visibly shows lyric status.
    Defaults[.enableLyrics] = true
