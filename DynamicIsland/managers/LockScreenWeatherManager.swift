...
// MARK: - Atoll Bridge Location Writer

/// Standalone CoreLocation writer for the external atoll_bridge.py daemon.
///
/// The daemon renders the notch weather card but runs under launchd, where it
/// cannot obtain location authorization. The app (a real GUI bundle with
/// NSLocationWhenInUseUsageDescription) resolves the device's true location —
/// WiFi-based, immune to proxy/VPN IP-geolocation errors — reverse geocodes it,
/// and writes /tmp/atoll_bridge_location.json atomically every 10 minutes:
///   {"latitude":…,"longitude":…,"label":"昆明官渡","ts":…,"source":"corelocation"}
/// The daemon prefers this file over its own config/default location.
@MainActor
final class AtollBridgeLocationWriter: NSObject, CLLocationManagerDelegate {
    static let shared = AtollBridgeLocationWriter()

    private let outputPath = "/tmp/atoll_bridge_location.json"
    private let refreshInterval: TimeInterval = 600  // 10 min; weather-grade cadence
    private let manager = CLLocationManager()
    private var timer: Timer?
    private var lastWrittenLabel: String?

    private override init() {
        super.init()
        manager.delegate = self
        manager.desiredAccuracy = kCLLocationAccuracyThreeKilometers  // weather-grade, low power
        start()
    }
    ...
    private func requestFix() {
        // macOS grants .authorizedAlways only; 'when in use' is iOS-only.
    ...
    nonisolated func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        NSLog("AtollBridgeLocationWriter: location error - \(error.localizedDescription)")
    }

    private func handle(_ location: CLLocation) {
        let lat = location.coordinate.latitude
        let lon = location.coordinate.longitude
        // Persist the fix immediately with the previous label, then refine via
        // reverse geocoding so the daemon never waits on the network.
        write(latitude: lat, longitude: lon, label: lastWrittenLabel)
        Task {
            let label = await reverseGeocode(location) ?? lastWrittenLabel
            if let label { lastWrittenLabel = label }
            write(latitude: lat, longitude: lon, label: label)
        }
    }
    ...
    private func reverseGeocode(_ location: CLLocation) async -> String? {
        do {
            ...
            let city = stripSuffix(p.locality ?? p.administrativeArea ?? "")
            let district = stripSuffix(p.subLocality ?? "")
            var label = city + district
            if label.isEmpty { label = stripSuffix(p.name ?? "") }
    ...
    /// "昆明市" -> "昆明", "官渡区" -> "官渡" (keeps non-Chinese names intact).
    private func stripSuffix(_ s: String) -> String {
        var out = s
        for suffix in ["市", "区", "县", "州"] where out.hasSuffix(suffix) && out.count > suffix.count {
            out = String(out.dropLast(suffix.count))
        }
        return out
    }

    private func write(latitude: Double, longitude: Double, label: String?) {
        var obj: [String: Any] = [
            "latitude": latitude,
            "longitude": longitude,
        ...
        let tmpPath = outputPath + ".tmp.\(ProcessInfo.processInfo.processIdentifier)"
        do {
