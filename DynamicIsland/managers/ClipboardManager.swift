...
private extension ClipboardItem {
    var reactionSummary: String {
        ...
        guard !cleanPreview.isEmpty else { return kind }
        return "\(kind) · \(String(cleanPreview.prefix(18)))"
    }
}
...
class ClipboardManager: ObservableObject {
    ...
    private func checkClipboard() {
        ...
        if !clipboardHistory.contains(where: { isSameContent($0, clipboardItem) }) {
            ...
            ReactionAnimationPlayer.shared.present(
                .clipboard,
                caption: "剪贴板已保存",
                detail: clipboardItem.reactionSummary,
                duration: 2.2
            )
        }
    }
