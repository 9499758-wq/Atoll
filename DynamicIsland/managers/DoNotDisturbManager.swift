...
final class DoNotDisturbManager: ObservableObject {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if usableIdentifier == nil && usableName == nil {
                if let isActive = isActive, isActive != previousActive {
                    ...
                    debugPrint("[DoNotDisturbManager] Status active-only update -> source: \(source) | isActive: \(isActive)")
                }
                return
            }
            ...
            if resolvedMode == .custom, !FullDiskAccessAuthorization.hasPermission() {
                ...
                case "default", "dnd", "do-not-disturb", "do not disturb", "donotdisturb":
                    finalName = FocusModeType.doNotDisturb.displayName
                default:
                    finalName = name
                }
            ...
            // If the status remains active and the mode switches, trigger an ON toast for the new mode.
            ...
            if identifierChanged || nameChanged || shouldToggleActive {
                debugPrint("[DoNotDisturbManager] Status update -> source: \(source) | identifier: \(trimmedIdentifier ?? "<nil>") | name: \(trimmedName ?? "<nil>") | resolved: \(resolvedMode.rawValue)")
            }
            ...
            // If the status turned OFF, retain metadata briefly for the OFF toast,
            ...
            if isActive == false {
                ...
                // Status turned ON; cancel any pending metadata clear and toast dismiss.
        ...
        stateVerificationTask = Task { @MainActor [weak self] in
            ...
            while !Task.isCancelled {
                ...
                if !focusStillActive {
                    ...
                    self.scheduleMetadataClear()
                    self.beginFocusToastDismissIfNeeded()
                    debugPrint("[DoNotDisturbManager] State verification: status no longer active, resetting.")
                    break
                }
            }
        }
    ...
    private func extractMetadata(from notification: Notification) -> (name: String?, identifier: String?) {
        ...
        debugPrint("[DoNotDisturbManager] raw status payload -> name: \(notification.name.rawValue), object: \(String(describing: notification.object)), userInfo: \(String(describing: notification.userInfo))")
...
enum FocusModeType: String, CaseIterable {
    ...
    var displayName: String {
        ...
        case .reduceInterruptions: return "减少打扰"
        case .custom: return ""
        case .unknown: return ""
        }
    }
...
extension FocusModeType {
    ...
    static func resolve(identifier: String?, name: String?) -> FocusModeType {
        ...
        // instead of incorrectly falling back to a generic system status.
        if let name, !name.isEmpty {
            return .custom
        }
...
private final class FocusLogStream {
    ...
    func start() {
        queue.async { [weak self] in
            ...
            do {
                ...
                debugPrint("[FocusLogStream] Started unified log tail for status metadata")
            } catch {
    ...
    private func handleTermination(terminateProcess: Bool = false) {
        ...
        isRunning = false
        clearMetadata()
        debugPrint("[FocusLogStream] Stopped unified log tail for status metadata")
    }
