...
class BluetoothAudioManager: ObservableObject {
    ...
    private var shouldMonitorBluetoothFeatures: Bool {
        Defaults[.showBluetoothDeviceConnections]
            || Defaults[.lockScreenBatteryShowsBluetooth]
            || Defaults[.showAirPodsListeningModeChanges]
    }
    ...
    private func updateMonitoringState() {
        if shouldMonitorBluetoothFeatures {
    ...
    private func startMonitoringIfNeeded() {
        ...
        bootstrapInitialDevicesInBackground()
        startPollingForChanges()
    }

    private func stopMonitoringIfNeeded() {
        guard isMonitoringStarted else { return }
        cleanup()
    ...
    /// 启动加固：IOBluetooth 栈偶发初始化死锁(实测卡住主线程导致整个 App 无窗口)。
    /// 初始设备探测挪到后台队列执行，主线程启动不再被蓝牙栈阻塞；
    /// 状态赋值统一回主线程，行为与原同步版一致。
    private func bootstrapInitialDevicesInBackground() {
    ...
    private func checkInitialDevices() {
        ...
        let detected = connectedAudioDevices.compactMap { device in
            createBluetoothAudioDevice(from: device)
        }

        // 可能由后台线程进入（见 bootstrapInitialDevicesInBackground）：
        // 蓝牙读取留在后台做，@Published 状态变更必须回主线程。
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.connectedDevices = detected
            // Update connection state
            self.isBluetoothAudioConnected = !detected.isEmpty
            self.refreshBatteryLevelsForConnectedDevices()
            if let lastDevice = detected.last {
