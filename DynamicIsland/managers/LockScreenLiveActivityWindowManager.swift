...
class LockScreenLiveActivityWindowManager {
    ...
    private var hostingView: PassthroughHostingView<LockScreenLiveActivityOverlay>?
    ...
    private func present(notchSize: CGSize, on screen: NSScreen) {
        ...
        if let hostingView {
            ...
            let view = PassthroughHostingView(rootView: overlayView)
            view.frame = CGRect(origin: .zero, size: targetFrame.size)
            view.contentCornerRadius = 18
            view.contentSize = targetFrame.size
            hostingView = view
