...
final class TimerControlWindowManager {
    ...
    private var hostingView: PassthroughHostingView<TimerControlOverlay>?
    ...
    private func ensureHostingView(with overlay: TimerControlOverlay) -> PassthroughHostingView<TimerControlOverlay> {
        if let hostingView {
            hostingView.rootView = overlay
            return hostingView
        }
        let view = PassthroughHostingView(rootView: overlay)
        view.contentCornerRadius = 18
        view.contentSize = CGSize(width: 220, height: 64)
        hostingView = view
        return view
    }
    ...
    private func measuredSize(for hosting: PassthroughHostingView<TimerControlOverlay>) -> CGSize {
