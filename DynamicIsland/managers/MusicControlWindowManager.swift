...
final class MusicControlWindowManager {
    ...
    private var hostingView: PassthroughHostingView<MusicControlOverlay>?
    ...
    private func ensureHostingView(with overlay: MusicControlOverlay) -> PassthroughHostingView<MusicControlOverlay> {
        if let hostingView {
        ...
        view.contentSize = CGSize(width: 240, height: 68)
        hostingView = view
        return view
    }
    ...
    private func measuredSize(for hosting: PassthroughHostingView<MusicControlOverlay>) -> CGSize {
