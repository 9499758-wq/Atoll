...
class DynamicIslandViewCoordinator: ObservableObject {
    ...
    func toggleSneakPeek(
        ...
        let reactionKind = reactionAnimationKind(for: type)

        if !isExtensionType && !bypassedTypes.contains(type) && !Defaults[.enableSystemHUD] {
            if status, let reactionKind {
                DispatchQueue.main.async {
                    ReactionAnimationPlayer.shared.present(reactionKind)
                }
            }
            return
        }
    ...
    private func reactionAnimationKind(for type: SneakContentType) -> ReactionAnimationKind? {
        switch type {
        case .brightness, .backlight: return .brightness
        ...
        case .bluetoothAudio: return .bluetooth
        case .privacy: return .privacy
        case .lockScreen: return .lock
        case .capsLock: return .capslock
        case .extensionLiveActivity: return .app
        }
    }
